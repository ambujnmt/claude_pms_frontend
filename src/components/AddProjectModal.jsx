    ...
    try {
      const payload = {
        ...
        milestones: form.milestones
          ...
          .map(m => ({ name: m.name, due_date: m.dueDate || null })),
        payments: form.payments
          .filter(p => p.amount && p.date)
          .map(p => ({ amount: parseFloat(p.amount), type: p.type, date: p.date, notes: p.notes, status: 'upcoming' })),
      };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field label="Project Name" required error={errors.name}>
              <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. E-Commerce Platform" style={inputStyle(errors.name)} />
            </Field>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            ...
            <Field label="Accent Colour">
              <div style={{ display: 'flex', gap: 6, paddingTop: 5 }}>
                {COLORS.map(c => (
                  <div key={c} onClick={() => set('color', c)} style={{ width: 22, height: 22, borderRadius: 6, background: c, cursor: 'pointer', border: form.color === c ? '3px solid var(--text)' : '2px solid transparent', boxSizing: 'border-box' }} />
                ))}
              </div>
            </Field>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <Field label="End Date" required error={errors.endDate}>
              <input type="date" value={form.endDate} onChange={e => set('endDate', e.target.value)} style={inputStyle(errors.endDate)} />
            </Field>
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {form.milestones.map((m, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1B2E6B', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#fff', flexShrink: 0 }}>{i + 1}</div>
              ...
              {form.milestones.length > 1 && (
                <button onClick={() => setForm(f => ({ ...f, milestones: f.milestones.filter((_, idx) => idx !== i) }))} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', padding: 4 }}><Trash2 size={13} /></button>
              )}
            </div>
          ))}
          <button onClick={() => setForm(f => ({ ...f, milestones: [...f.milestones, { name: '', dueDate: '' }] }))} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 7, border: '1.5px dashed var(--border-bright)', background: 'var(--bg-elevated)', color: 'var(--text-muted)', fontSize: 13, cursor: 'pointer', width: 'fit-content' }}>
          ...
          {form.payments.map((p, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 8, alignItems: 'center' }}>
              ...
              {form.payments.length > 1 && (
                <button onClick={() => setForm(f => ({ ...f, payments: f.payments.filter((_, idx) => idx !== i) }))} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', padding: 4 }}><Trash2 size={13} /></button>
              )}
            </div>
          ))}
          <button onClick={() => setForm(f => ({ ...f, payments: [...f.payments, { amount: '', date: '', type: 'Milestone', notes: '' }] }))} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 7, border: '1.5px dashed var(--border-bright)', background: 'var(--bg-elevated)', color: 'var(--text-muted)', fontSize: 13, cursor: 'pointer', width: 'fit-content' }}>
