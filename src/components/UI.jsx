...
export function Card({ children, style = {}, hover, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      boxShadow: 'var(--shadow-sm)', transition: 'all 0.16s',
...
// ─── Buttons ─────────────────────────────────────────────────────
export function Btn({ children, onClick, variant='primary', size='md', icon, style:sx={}, disabled=false, type='button' }) {
  const base = { display:'inline-flex', alignItems:'center', gap:6, border:'none', cursor:disabled?'not-allowed':'pointer', fontFamily:'var(--font-body)', fontWeight:600, borderRadius:7, transition:'all 0.14s', opacity:disabled?0.5:1, letterSpacing:'-0.1px' };
  const sizes = { sm:{padding:'5px 12px',fontSize:12}, md:{padding:'7px 16px',fontSize:14}, lg:{padding:'10px 22px',fontSize:14} };
  const variants = {
    primary:{ background:'#2E6DB4', color:'#fff', boxShadow:'0 1px 4px rgba(46,109,180,0.25)' },
    ghost:  { background:'var(--bg-elevated)', color:'var(--text-muted)', border:'1px solid var(--border)' },
    danger: { background:'var(--danger)', color:'#fff' },
    success:{ background:'var(--success)', color:'#fff' },
    outline:{ background:'transparent', color:'#2E6DB4', border:'1.5px solid #2E6DB4' },
    navy:   { background:'#1B2E6B', color:'#fff' },
  };
  return (
...
export const inputStyle = (err) => ({
  ...
  borderRadius:7, background:'var(--bg-surface)', color:'var(--text)', outline:'none',
  transition:'border-color 0.15s',
});

// ─── Badges ──────────────────────────────────────────────────────
export function Badge({ label, color='#2E6DB4', bg }) {
  return (
    <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 8px', borderRadius:5, fontSize:11, fontWeight:600, background:bg||`${color}15`, color, border:`1px solid ${color}28`, whiteSpace:'nowrap', letterSpacing:'0.1px' }}>{label}</span>
  );
}
...
export function StatusBadge({ status }) {
  const m = {
    ...
    completed:     { color:'#2E6DB4',        label:'Completed' },
    'on-hold':     { color:'var(--warning)', label:'On Hold' },
    overdue:       { color:'var(--danger)',  label:'Overdue' },
    'in-progress': { color:'#4A90D9',        label:'In Progress' },
    upcoming:      { color:'var(--text-muted)', label:'Upcoming' },
    pending:       { color:'var(--warning)', label:'Pending' },
    received:      { color:'var(--success)', label:'Received' },
    'due-soon':    { color:'var(--orange)',  label:'Due Soon' },
...
// ─── Progress ────────────────────────────────────────────────────
export function ProgressBar({ value, color='#2E6DB4', height=6, showLabel, bg }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
      ...
      {showLabel && <span style={{ fontSize:12, fontWeight:600, color, minWidth:32, textAlign:'right', fontFamily:'var(--font-mono)' }}>{value}%</span>}
    </div>
  );
}
      ...
      <table style={{ width:'100%', borderCollapse:'collapse' }}>
        <thead>
          <tr style={{ background:'#1B2E6B' }}>
            {headers.map(h => (
              <th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:11, fontWeight:700, color:'rgba(168,206,236,0.9)', borderBottom:'none', whiteSpace:'nowrap', textTransform:'uppercase', letterSpacing:'0.5px' }}>{h}</th>
            ))}
          </tr>
        </thead>
...
export function TD({ children, style }) {
  return <td style={{ padding:'10px 14px', fontSize:13, verticalAlign:'middle', ...style }}>{children}</td>;
}
  ...
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:20, paddingBottom:14, borderBottom:'2px solid #2E6DB4' }}>
      <div>
        ...
        {sub && <p style={{ fontSize:13, color:'var(--text-muted)', marginTop:6 }}>{sub}</p>}
      </div>
      {action}
    </div>
  );
...
export function EmptyState({ icon='📂', message }) {
  return <div style={{ padding:28, textAlign:'center', color:'var(--text-muted)', fontSize:14 }}><div style={{ fontSize:26, marginBottom:8 }}>{icon}</div>{message}</div>;
}
...
export function ActionMenu({ onEdit, onDelete }) {
  return (
    <div style={{ display:'flex', gap:4 }}>
      <button onClick={e => { e.stopPropagation(); onEdit(); }} style={{ padding:'4px 10px', borderRadius:5, border:'1px solid var(--border)', background:'var(--bg-elevated)', fontSize:12, cursor:'pointer', color:'#2E6DB4', fontWeight:600 }}>Edit</button>
      <button onClick={e => { e.stopPropagation(); onDelete(); }} style={{ padding:'4px 10px', borderRadius:5, border:'1px solid #9B1C1C28', background:'var(--danger-dim)', fontSize:12, cursor:'pointer', color:'var(--danger)', fontWeight:600 }}>Delete</button>
    </div>
  );
}
...
export function formatAmount(value, currency) {
  ...
  if (currency?.useLakhSystem) {
    // Indian system: L (lakh = 100,000), K (thousand)
...
export function CategoryColor(cat) {
  return { Website:'#2E6DB4', 'Mobile App':'#4C3A9E', 'AI/ML':'#A85010' }[cat] || 'var(--text-muted)';
}
