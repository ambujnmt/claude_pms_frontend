...
export default function HostingPage() {
  ...
  const handleDelete = async () => {
    try { await hostingService.delete(confirmDel); setHosting(prev=>prev.filter(h=>h.id!==confirmDel)); }
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:18 }}>
        {[
          ...
          <Card key={s.label} style={{ display:'flex', alignItems:'center', gap:12, padding:14 }}>
            ...
            <div><div style={{ fontSize:22, fontWeight:700, color:s.color, lineHeight:1 }}>{s.val}</div><div style={{ fontSize:14, color:'var(--text-muted)', marginTop:3 }}>{s.label}</div></div>
          </Card>
        ))}
      </div>

      <Card style={{ padding:0, overflow:'hidden', marginBottom:18 }}>
        <div style={{ padding:'13px 20px', borderBottom:'1px solid var(--border)', fontSize:15, fontWeight:700, color:'#1B2E6B' }}>All Hosted Clients</div>
        {dataLoading
              ...
              {sorted.map(h => {
                ...
                return (
                  <TR key={h.id} highlighted={h.status==='overdue'}>
                    <TD><div style={{ fontWeight:700 }}>{h.clientName}</div><div style={{ fontSize:12, color:'var(--text-muted)' }}>{h.contactEmail}</div></TD>
        ...
        {!dataLoading && sorted.length===0 && <EmptyState message="No hosting records yet."/>}
      </Card>
        ...
        >
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
              <Field label="Plan"><select value={form.plan} onChange={e=>setForm(f=>({...f,plan:e.target.value}))} style={inputStyle()}>{PLANS.map(p=><option key={p}>{p}</option>)}</select></Field>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
              <Field label="Server"><select value={form.server} onChange={e=>setForm(f=>({...f,server:e.target.value}))} style={inputStyle()}>{SERVERS.map(s=><option key={s}>{s}</option>)}</select></Field>
              <Field label="Status"><select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))} style={inputStyle()}>{STATUS_OPTS.map(s=><option key={s}>{s}</option>)}</select></Field>
            </div>
            ...
            <Field label="Add-ons">
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {ADDONS.map(a=><div key={a} onClick={()=>toggleAddon(a)} style={{ padding:'4px 10px', borderRadius:7, cursor:'pointer', fontSize:13, fontWeight:600, border:`1.5px solid ${form.addons.includes(a)?'#2E6DB4':'var(--border)'}`, background:form.addons.includes(a)?'var(--accent-dim)':'var(--bg-elevated)', color:form.addons.includes(a)?'#2E6DB4':'var(--text-muted)' }}>{a}</div>)}
              </div>
            </Field>
          </div>
        </Modal>
      ...
      {confirmDel && <ConfirmModal message="Delete this hosting record?" onConfirm={handleDelete} onCancel={() => setConfirmDel(null)}/>}
    </div>
  );
}
